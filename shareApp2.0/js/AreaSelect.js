...
//			this.text = this.settings.text || this.trigger.val().split(' ') || [' ', ' ', ' '];
		...
		bindEvent: function() {
			...
			this.trigger[_this.settings.eventName](function(e) {
				...
				$.confirm('<div class="ad-scroller-mask"><div id="' + _this.id + '" class="ad-scroller">' + dlgContent + '<p></p></div></div>', null, function(t, c) {
					if (t == "yes") {
						_this.submit()
					}
					if (t == 'no') {
		...
		submit: function() {
			...
			if (this.trigger[0].nodeType == 1) {
				...
				this.trigger.attr('data-info', this.text.join(','));
			}
