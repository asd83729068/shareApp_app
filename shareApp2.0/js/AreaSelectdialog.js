...
(function(root, factory) {
	...
	$.Dialog = function(settings) {
		if (settings.type === "alert") {
			...
			var html = '<div class="ad-alert-title">' + settings.content + '</div>';
			var action = '';
			if (settings.button) {
				if (typeof settings.button == 'boolean') {
					settings.button = '确定';
				};
				action = '<p class="ad-dialog-action"><button class="ad-alert-submit  js-dialog-close">' + settings.button + '</button></p>';
		...
		if (settings.type === "confirm") {
			var dialog = new Dialog();
			var html = '<div class="ad-confirm-title">' + settings.content + '</div>';
			var action = '';
			if (!settings.buttons) {
				settings.buttons = [{
					'yes': '确定'
			...
			for (var i = 0, l = settings.buttons.length; i < l; i++) {
				...
				if (item.yes) {
					btnstr += '<td><button class="ad-confirm-submit " data-type="yes">' + item.yes + '</button></td>';
				}
				if (item.no) {
					btnstr += '<td><button class="ad-confirm-no" data-type="no">' + item.no + '</button></td>';
				}
				if (item.close) {
					btnstr += '<td><button class="ad-confirm-close js-dialog-close" data-type="close">' + item.close + '</button></td>';
				}
			}
			action = '<table class="ad-dialog-action"><tr>' + btnstr + '</tr></table>';
			html += action;
			var options = $.extend({
				target: html,
				animate: true,
				...
				beforeShow: function(c) {
					dialog.touch($('.ad-confirm-submit', c), function() {
						settings.callback && settings.callback.call(dialog, 'yes', c);
					});
					dialog.touch($('.ad-confirm-no', c), function() {
						settings.callback && settings.callback.call(dialog, 'no', c);
					});
					dialog.touch($('.ad-confirm-close', c), function() {
	...
	var Dialog = function() {
		...
		this.id = 'dialog_' + rnd;
		this.settings = {};
		this.settings.closeTpl = $('<span class="ad-dialog-close js-dialog-close">x</span>');
		this.settings.titleTpl = $('<div class="ad-dialog-title"></div>');
		this.timer = null;
		this.showed = false;
		this.mask = $();
	}
	Dialog.prototype = {
		init: function(settings) {
			...
			if (this.settings.mask) {
				this.mask = $('<div class="ad-dialog-mask"/>');
				$('body').append(this.mask);
			}
			$('body').append('<div class="ad-dialog" id="' + this.id + '"></div>');
